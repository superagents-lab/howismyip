...
sources at once. Is it a proxy / VPN / Tor exit / datacenter IP? What's its
ASN, ISP, geolocation, owning registry? Is it on any blocklists? Ask once, get
every source's answer plus a merged consensus.

Built with the **TanStack** full stack (Start + Router + server functions),
a geek/terminal aesthetic, English/中文 UI (URL-based, `/en` · `/zh`), and a
risk-detector comparison matrix that makes provider disagreement obvious. Ships
...
Six more activate automatically **when you provide an API key** (see
